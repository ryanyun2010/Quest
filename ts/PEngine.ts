...
export class RenderingEngine {
    ...
    constructor(){
        ...
        this.vertices = null;
        this.gameObjects = [new GameObject(new Rect(new Position(Math.random() * 2000 - 1000,Math.random() * 1500 - 750), 300, 300, new Color(Math.random() * 255,Math.random() * 255,Math.random() * 255,255)))];
    ...
    async renderPass(vertexBuffer: GPUBuffer): Promise<void>{
        if(this.device == null){throw new Error("Device not initialized, try calling initWebGPU() first");}
        if(this.renderPipeline == null){throw new Error("Render Pipeline not initialized, try calling setupPipeline() first");}
        if(this.context == null){throw new Error("WebGPU Context not initialized, try calling setupContext() first");}
    ...
    async setupShaderModule(): Promise<void>{
        if(this.device == null){throw new Error("Device not initialized.");}
    ...
    async setupContext(): Promise<void>{
        ...
        if(this.canvas == null){throw new Error("Couldn't find canvas, do you have a #gpuCanvas in your HTML?");}
        this.context = this.canvas.getContext("webgpu");
        if(this.context == null){throw new Error("Couldn't initialize WebGPU Context");}
        if(this.device == null){throw new Error("Device not initialized, try calling setupContext() first");}
    ...
    async setupPipeline(): Promise<void>{
        if(this.shaderModule == null){throw new Error("Shader Module not initialized, try calling setupShaderModule() first");}
        if(this.vertexBufferLayout == null){throw new Error("Vertex Buffer layout not initialized, try calling setupVertexBuffers() first");}
        if(this.device == null){throw new Error("Device not initialized, try calling initWebGPU() first");}
